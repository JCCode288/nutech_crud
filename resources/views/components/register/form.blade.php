    ...
    <form method="post" action='/register'>
        ...
        <div class="grid grid-cols-1 gap-6 mt-4 sm:grid-cols-2 w-fit h-fit">
            ...
            <div>
                ...
                <input id="name" type="name" name="name" class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring">
                @error('name')
                    <x-exception.text :message="$message" />
                @enderror
            </div>
        ...
        <a class="text-gray-700 dark:text-gray-200" href="/login">Already have an account? Login here.</a>

        <div class="flex justify-end mt-6">
            <button class="px-8 py-2.5 leading-5 text-white transition-colors duration-300 transform bg-gray-700 rounded-md hover:bg-gray-600 focus:outline-none focus:bg-gray-600" type="submit">Register</button>
        </div>
    </form>
    @session('error')
        <div>
